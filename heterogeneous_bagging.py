...
def balanced_bootstrap_sampling(data, label, emotion_name_list):
	bagging_train_data = []
	bagging_train_label = []
	a, e, n, p ,r = separa_fau_five(data, label)
...
def skewed_bootstrap_sampling(data, label, emotion_name_list):
	bagging_train_data = []
	bagging_train_label = []
	a, e, n, p ,r = separa_fau_five(data, label)
	bagging_train_size = np.zeros(5)
...
def missing_data(data, label, ratio):
	missing_index = []
...
def random_feature_subsets(train, test, model):
	if model == 'MLP' or model == 'CNN':
		miss_feature = np.random.choice(train.shape[-1], size=24, replace=False)
		train = np.delete(train, miss_feature, axis=1)
		test = np.delete(test, miss_feature, axis=1)
	elif model == 'RNN':
		miss_feature = np.random.choice(train.shape[-1], size=7, replace=False)
		train = np.delete(train, miss_feature, axis=2)
		test = np.delete(test, miss_feature, axis=2)
	return train, test
...
def train_MLP(train_data, train_label, test_data, test_label, test_size, data_weights, bagging_model_path=''):
	...
	#model.load_weights('./static_model/mlp_ts_46_model.h5')
	if args.load_model == '':
		adam = Adam(lr=args.learning_rate, decay=args.decay)
		#adam = Adam()
		model.compile(loss=weighted_loss(r=data_weights), optimizer=adam, metrics=['accuracy'])
...
if __name__ == '__main__':
	...
	data_weights = np.asarray(data_weights)
	classifier_count = np.zeros(3) #index 0: RNN , index 1: CNN , index 2: mlp_input
	for i in range(args.bagging_iters):
		#random_base_classifier = np.random.choice(['RNN', 'CNN', 'MLP'], p=[0.5, 0.2, 0.3])
		random_base_classifier = np.random.choice(['RNN', 'CNN', 'MLP'])
		if random_base_classifier == 'RNN':
			print('=====================================================================')
			print ('<< Heterogeneous Bagging training procedure for RNN >>')
			classifier_count[0] += 1
			...
			if args.sample_scheme[0] == 'b':
				bag_dynamic_train_data, bag_train_label, bag_train_size = balanced_bootstrap_sampling(dynamic_train_data, train_label, emotion_name_list)
			elif args.sample_scheme[0] == 'u':
				bag_dynamic_train_data, bag_train_label, bag_train_size = skewed_bootstrap_sampling(dynamic_train_data, train_label, emotion_name_list)
			...
			#bag_dynamic_train_data, bag_train_label = missing_data(bag_dynamic_train_data, bag_train_label, 0.1)
			#print ('After MCAR missing: {0}, {1}'.format(bag_dynamic_train_data.shape , bag_train_label.shape))
			# bag_dynamic_train_data, dynamic_test_data_subsets = random_feature_subsets(bag_dynamic_train_data, dynamic_test_data, model='RNN')
			# print ('After feature missing-> train: {0}, test: {1}'.format(bag_dynamic_train_data.shape , dynamic_test_data_subsets.shape))
			...
			print('=====================================================================')
		elif random_base_classifier == 'CNN':
			print('=====================================================================')
			print ('<< Heterogeneous Bagging training procedure for CNN >>')
			classifier_count[1] += 1
			...
			print ('\nConstruct dataset for classifier {}/{}........'.format(i+1, args.bagging_iters))
			if args.sample_scheme[0] == 'b':
				bag_static_train_data, bag_train_label, bag_train_size = balanced_bootstrap_sampling(static_train_data, train_label, emotion_name_list)
			elif args.sample_scheme[0] == 'u':
				bag_static_train_data, bag_train_label, bag_train_size = skewed_bootstrap_sampling(static_train_data, train_label, emotion_name_list)
				data_weights = adjust_data_weights(bag_static_train_data, bag_train_label)
			print ('Data sizes for each class: {}'.format(bag_train_size))
			#bag_static_train_data, bag_train_label = missing_data(bag_static_train_data, bag_train_label, 0.1)
			#print ('After MCAR missing: {0}, {1}'.format(bag_static_train_data.shape , bag_train_label.shape))
			# bag_static_train_data, static_test_data_subsets = random_feature_subsets(bag_static_train_data, static_test_data, model='CNN')
			# print ('After feature missing-> train: {0}, test: {1}'.format(bag_static_train_data.shape , static_test_data_subsets.shape))
			# reshape the data to fit the CNN model
			bag_static_cnn_train_data = np.reshape(bag_static_train_data, (bag_static_train_data.shape[0], args.llds, args.functionals, args.delta))
			static_cnn_test_data = np.reshape(static_test_data, (static_test_data.shape[0], args.llds, args.functionals, args.delta))

			predict = train_CNN(bag_static_cnn_train_data, bag_train_label, static_cnn_test_data, test_label, test_size, data_weights, args.model_root_dir)
			predicts[i] = predict
			#args.model_root_dir = temp	
			print('=====================================================================')

		elif random_base_classifier == 'MLP':
			print('=====================================================================')
			print ('<< Heterogeneous Bagging training procedure for MLP >>')
			classifier_count[2] += 1
			...
			#bag_static_train_data, bag_train_label = missing_data(bag_static_train_data, bag_train_label, 0.1)
			#print ('After MCAR missing: {0}, {1}'.format(bag_static_train_data.shape , bag_train_label.shape))
			# bag_static_train_data, static_test_data_subsets = random_feature_subsets(bag_static_train_data, static_test_data, model='MLP')
			# print ('After feature missing-> train: {0}, test: {1}'.format(bag_static_train_data.shape , static_test_data_subsets.shape))
